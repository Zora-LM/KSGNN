...
def gen_feature(path, name, featurizer):
    ...
    node_num = pocket_atom_num_from_mol2(path, name)  # number of atoms in a pocket
...
def cons_lig_pock_graph_with_spatial_context(args, ligand, pocket, add_fea=2, theta=5, theta2=4, keep_pock=False, pocket_spatial=True):
    ...
    return lig_size, size, coords, feats, feats_dict, edges, dist, atoms_raw


# prepare bond nodes #
def bond_node_prepare(mol, cut_dist):
    num_atoms_d, num_atoms, coords, features, features_dict, edges, length, atoms = mol
...
def prepare_ssl_task(data):
    ...
    data['Bl_bond_length'] = np.array(data['bond_len'])
...
def mol2graph(args, mol_path, cutoff, add_fea, name, training):
    ...
    if args.init_emb:
        for n in fea_name_list:
            if n == 'partialcharge':
                features[n] = torch.FloatTensor(features[n])
            else:
                features[n] = torch.LongTensor(features[n])
    else:
    ...
    if (training & args.is_mask):
        masked_g = mask_geognn_graph(g, mask_ratio=args.mask_ratio, graph_type='bab')

    if (training & args.is_ssl):
        data = prepare_ssl_task(data)  # self-supervised learning tasks

    if training:
        return g, bab_g, masked_g, data
    else:
        return g, bab_g
