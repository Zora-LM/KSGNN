...
class GATLayer(nn.Module):
    def __init__(self, args, in_feats, out_feats, activation, layer_norm=False):
        ...
        self.args = args
        self.n_head = 1 #.n_head
        self.layer_norm = layer_norm
        self.attn_dim = in_feats // self.n_head
        if layer_norm:
            self.head_norm = LayerNorm(in_feats)
            self.tail_norm = LayerNorm(in_feats)
            if args.pool_rel:
                self.rel_norm = LayerNorm(in_feats)
        self.feat = nn.Linear(in_feats, in_feats, bias=False)
        self.fc_h = nn.Linear(in_feats, in_feats, bias=False)
        self.fc_t = nn.Linear(in_feats, in_feats, bias=False)
        self.attn_h = nn.Parameter(torch.FloatTensor(size=(1, self.n_head, self.attn_dim)), requires_grad=True)
        self.attn_t = nn.Parameter(torch.FloatTensor(size=(1, self.n_head, self.attn_dim)), requires_grad=True)
        if args.pool_rel:
            self.fc_r = nn.Linear(in_feats, in_feats, bias=False)
            self.attn_r = nn.Parameter(torch.FloatTensor(size=(1, self.n_head, self.attn_dim)), requires_grad=True)
    ...
    def reset_parameters(self):
        ...
        nn.init.xavier_normal_(self.fc_h.weight.data, gain=1.414)
        nn.init.xavier_normal_(self.fc_t.weight.data, gain=1.414)
        nn.init.xavier_normal_(self.feat.weight.data, gain=1.414)
        nn.init.xavier_normal_(self.attn_t, gain=1.414)
        nn.init.xavier_normal_(self.attn_h, gain=1.414)
        if self.args.pool_rel:
            nn.init.xavier_normal_(self.fc_r.weight.data, gain=1.414)
    ...
    def forward(self, g, h, r):
        ...
        def edge_attention_no_rel(edges):
            return {'e': self.act(edges.src['eh']) + self.act(edges.dst['et'])}
        ...
        if self.layer_norm:
            ...
            if self.args.pool_rel:
                er = self.rel_norm(er)
        feat = self.feat(h).view(-1, self.n_head, self.attn_dim)
        feat_head = torch.tanh(self.fc_h(eh).view(-1, self.n_head, self.attn_dim))
        feat_tail = torch.tanh(self.fc_t(et).view(-1, self.n_head, self.attn_dim))
        eh = (feat_head * self.attn_h).sum(dim=-1).unsqueeze(-1)
        et = (feat_tail * self.attn_t).sum(dim=-1).unsqueeze(-1)
        g.ndata.update({'ft': feat, 'eh': eh, 'et': et})
        if self.args.pool_rel:
            feat_rel = torch.tanh(self.fc_r(er).view(-1, self.n_head, self.attn_dim))
        ...
        feat = g.ndata.pop('ft').view(-1, self.n_head*self.attn_dim)
        h = self.fc(feat)
...
class DiffPoolBatchedGraphLayer(nn.Module):
    ...
    def __init__(self, args, input_dim, assign_dim, output_feat_dim, activation, dropout, aggregator_type):
        ...
        if self.args.pool_gnn == 'graphsage':
            ...
            self.pool_gc = GATLayer(args, input_dim, assign_dim, activation, layer_norm=args.gat_ln)
        elif self.args.pool_gnn == 'gcn':
            self.pool_gc = GCNLayer(input_dim, assign_dim, activation)
...
class BatchedGraphSAGE(nn.Module):
    def __init__(
        self, infeat, outfeat, use_bn=True, mean=False, add_self=False, bn_dim=None
    ):
        super().__init__()
