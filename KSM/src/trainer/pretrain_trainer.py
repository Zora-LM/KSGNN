...
class Trainer():
    ...
    def _forward_epoch(self, model, batched_data):
        a2a_graphs, bab_graphs, pk_values, fps, mds, strings, seqs = batched_data
        a2a_graphs = a2a_graphs.to(self.device)
        bab_graphs = bab_graphs.to(self.device)
        ...
        if self.args.smiles_feat == 'fp':
            ...
            strings = strings.to(self.device)

        results = model(a2a_graphs, bab_graphs, fps=fps, mds=mds, strings=strings, seqs=seqs)
    ...
    def _eval(self, model, batched_data):
        ...
        preds = model(a2a_graphs, bab_graphs, fps=fps, mds=mds, strings=strings, seqs=seqs)
    ...
    def train_epoch(self, model, train_loader, epoch_idx):
        ...
        if self.summary_writer is not None:
            self.summary_writer.add_scalar('Train/total_loss', total_loss, epoch_idx)
    ...
    def fit(self, model, train_loader, val_loader, test_loader):
        ...
        for epoch in tqdm(range(self.args.epoch_run, self.args.n_epoch)):
            ...
            if best_loss > val_loss:
                ...
                count = 0
                self.save_snapshot(model, epoch, step=self.lr_scheduler._step_count)
            elif (abs(best_loss - val_loss) < 0.1) and (train_loss > 0.01):
                count = 0
            else:
                count += 1

            if (count == self.args.patience) | (epoch == self.args.n_epoch - 1) | (train_loss < 0.003):
